...
def test_generate_alt_returns_suggestion_payload(configured_client: TestClient) -> None:
    response = configured_client.post(
        ...
        json={
            ...
            "title": "零信任架构落地指南",
            "image_title": "网关与策略引擎拓扑图",
            "summary": "解释终端、网关、身份源与策略引擎之间的访问链路。",
        },
        headers={"X-Internal-Token": INTERNAL_TOKEN},
    )
    ...
    assert "网关与策略引擎拓扑图" in suggestion["patches"][0]["new_text"]
    assert "零信任架构落地指南" in suggestion["patches"][0]["new_text"]
    assert suggestion["source_chunks"][0]["source_type"] == "article"


def test_siliconflow_generate_alt_prompt_contains_image_and_article_context(monkeypatch: pytest.MonkeyPatch) -> None:
    test_settings = replace(
        base_settings,
        ai_provider="siliconflow",
        ai_model="siliconflow-vision-test",
        ai_prompt_version="v-test",
        siliconflow_api_key="test-key",
        internal_api_token=INTERNAL_TOKEN,
    )
    monkeypatch.setattr(providers_module, "settings", test_settings)
    provider = providers_module.SiliconFlowProvider()
    captured: dict[str, Any] = {}

    def fake_request_chat_completion(prompt: str, trace_id: str, model: str, temperature: float = 0.2) -> str:
        captured["prompt"] = prompt
        captured["trace_id"] = trace_id
        captured["model"] = model
        captured["temperature"] = temperature
        return '{"alt_text":"零信任拓扑图，展示终端通过网关访问策略引擎"}'

    monkeypatch.setattr(provider, "_request_chat_completion", fake_request_chat_completion)

    payload = {
        "article_id": 88,
        "title": "零信任架构落地指南",
        "summary": "解释终端、网关、身份源与策略引擎之间的访问链路。",
        "keywords": ["零信任", "网关", "策略引擎"],
        "content_html": "<p>文章讨论终端通过统一访问网关接入策略引擎。</p>",
        "image_url": "https://example.com/zero-trust.png",
        "image_title": "零信任拓扑图",
        "image_caption": "终端访问网关与策略引擎的调用关系",
        "image_context": "图中包含终端、身份源、访问网关和策略引擎四个节点",
        "prompts": {
            "alt_prompt": "请综合图片信息和文章上下文生成 alt。"
        },
    }

    result = provider.generate_alt(article_id=88, payload=payload, trace_id="trace_alt_ctx")

    assert captured["trace_id"] == "trace_alt_ctx"
    assert captured["model"] == "siliconflow-vision-test"
    assert captured["temperature"] == 0.2
    assert "alt_generation_context=" in captured["prompt"]
    assert "零信任架构落地指南" in captured["prompt"]
    assert "零信任拓扑图" in captured["prompt"]
    assert "图中包含终端、身份源、访问网关和策略引擎四个节点" in captured["prompt"]
    assert '"keywords": ["零信任", "网关", "策略引擎"]' in captured["prompt"]
    assert result["suggestion"]["patches"][0]["new_text"] == "零信任拓扑图，展示终端通过网关访问策略引擎"
...
def test_contract_openapi_file_still_contains_ai_service_paths() -> None:
    contract_text = Path(__file__).resolve().parents[3].joinpath("contracts/openapi.ai-service.yaml").read_text(encoding="utf-8")
