...
def sync_ecs_metrics(client, region_id, instance_ids):
    ...
    metrics = _env("ALIYUN_CMS_METRICS", "CPUUtilization,MemoryUtilization,InternetIn,InternetOut")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Auto-discover regions when not configured
        ...
        # Auto-discover DNS domains when not configured
        ...
        self.stdout.write("Syncing ECS instances...")
        for region in region_list:
            sync_ecs_instances(client, region['id'], region['endpoint'])

        self.stdout.write("Syncing DNS statistics...")
        sync_dns_stats(client, domain_list)

        self.stdout.write("Syncing ECS metrics...")
        for region in region_list:
            instance_ids = list(
                EcsInstanceSnapshot.objects.filter(region_id=region['id']).values_list("instance_id", flat=True)
            )
            sync_ecs_metrics(client, region['id'], instance_ids)

        self.stdout.write(self.style.SUCCESS("Aliyun sync completed."))
