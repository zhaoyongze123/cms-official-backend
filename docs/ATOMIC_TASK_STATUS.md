...
| A08 | DONE | `feature/tiptap-editor-basic` | TipTap 文档结构、blockId、content_json 保存、content_html 渲染 | A01、A07 | 2026-05-09 已在 `develop` 基线补完 Django `GET/PATCH /api/articles/{id}/`、TipTap `content_json/content_html` 保存链路，并完成 `npm run test`、`npm run lint`、`npm run build` 与 `docker compose run --rm web python manage.py test apps.simple_cms` 验证 |
| A09 | DONE | `feature/tiptap-diff-editor` | AI Diff 渲染、Accept、Reject、编辑后接受、content_hash 冲突 | A06、A08 | 2026-05-09 已补 Django suggestion 状态机、patch apply、`content_hash` 冲突检测、编辑后接受；前端编辑页已接入 AI 审核、建议列表、接受/拒绝按钮；验证包含 `python3 -m py_compile apps/simple_cms/api_services.py apps/simple_cms/api_views.py apps/simple_cms/tests.py`、`npm test`、`docker compose run --rm web python manage.py test apps.simple_cms` 共 45 项通过 |
| A10 | DONE | `feature/publish-flow` | 发布前检查、发布 API、Error 阻断、Warning 提示 | A03、A06、A09 | 2026-05-09 已补 `/api/articles/{id}/seo-check/` 与 `/api/articles/{id}/publish/`，实现 Error 阻断、Warning 透传；前端编辑页已接入发布前检查与发布按钮；验证包含 Django 单测通过，`publish_blocked` 与成功发布场景均有真实用例覆盖 |
| A11 | DONE | `feature/analytics-monitoring` | AnalyticsSnapshot、GSC/GA4 Stub、站内事件、监控面板 | A07 | 2026-05-09 已补 Django AnalyticsSnapshot、单篇监控与 SEO 总览 API，并将 Next.js 监控页改为直连 Django 聚合接口；验证包含 `npm run lint`、`npm test`、`npm run build` 通过 |
| A12 | DONE | `feature/e2e-integration` | 三端联调、契约回归、真实流程冒烟 | A03-A11 | 2026-05-09 已在 `develop` 完成 API 级 E2E 冒烟：`POST /api/articles/` -> `PATCH /api/articles/{id}/` -> `POST /api/articles/{id}/ai-review/` -> `POST /api/ai-suggestions/{id}/accept/` -> `POST /api/articles/{id}/seo-check/` -> `POST /api/articles/{id}/publish/` -> `GET /api/articles/{id}/analytics/` -> `GET /api/dashboard/seo-summary/`；真实结果为 `create 201 / patch 200 / review completed / accept accepted / seo_can_publish true / publish published / analytics_snapshot_count 1` |
